...
const getFileIcon = (fileName: string) => {
    ...
    switch(ext) {
        ...
        case 'md': return <FileIcon size={14} className="text-secondary" />;
...
export function ProjectPreviewModal({ 
                  ...
                  <div key={node.path}>
                      <div 
                        className={`
                            ...
                            ${node.isDir ? 'text-secondary hover:text-primary' : 
                              selectedFile === node.path ? 'bg-primary/20 text-primary' : 'text-secondary hover:text-primary hover:bg-secondary'}
                        `}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-primary rounded-xl border border-border w-full max-w-6xl h-[85vh] flex flex-col shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between p-4 border-b border-border bg-primary">
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className="p-2 hover:bg-secondary rounded-lg transition-colors text-secondary hover:text-primary flex items-center gap-2 text-sm"
              title="Download project ZIP"
            >
              <Download size={18} />
              <span className="hidden sm:inline text-xs font-medium">Download ZIP</span>
            </button>
            <button onClick={onClose} className="p-1 hover:bg-secondary rounded-lg transition-colors text-secondary hover:text-primary">
              <X size={20} />
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-1 min-h-0 relative" ref={containerRef}>
           ...
           {/* Sidebar - File Tree */}
           <div 
             className="border-r border-border bg-primary/50 flex flex-col min-h-0 shrink-0"
             style={{ width: `${currentSidebarWidth}px`, flexBasis: `${currentSidebarWidth}px` }}
           >
               <div className="p-3 text-xs font-semibold text-tertiary uppercase tracking-wider border-b border-border/50">
                   Explorer
               </div>
               <div className="flex-1 overflow-y-auto p-2 scrollbar-themed text-secondary">
                   {loading && !zip ? (
                       <div className="flex flex-col items-center justify-center h-40 text-tertiary">
           ...
           >
              <div className="w-[1px] h-8 bg-tertiary group-hover:bg-primary/50 invisible group-hover:visible flex items-center justify-center">
                 <GripVertical size={10} className="text-tertiary ml-[-4px]" />
              </div>
           </div>
           ...
           <div className="flex-1 flex flex-col min-h-0 bg-[#1e1e1e]">
              ...
              {/* File Tab */}
              {selectedFile && (
                  <div className="bg-[#2d2d2d] px-4 py-2 text-sm text-primary border-b border-black/20 flex items-center gap-2 shadow-sm">
                      {getFileIcon(selectedFile)}
                      <span>{selectedFile.split('/').pop()}</span>
                      <span className="text-xs text-tertiary ml-2 opacity-50">{selectedFile}</span>
                  </div>
              )}
              ...
              <div className="flex-1 overflow-auto relative scrollbar-themed">
                  {loading && !fileContent ? (
                       <div className="flex items-center justify-center h-full text-tertiary gap-2">
                      ...
                      selectedFile ? (
                        <pre className="!m-0 !p-6 !bg-transparent text-sm min-h-full font-mono !font-normal !whitespace-pre !w-max min-w-full">
                          ...
                          <div className="flex items-center justify-center h-full text-tertiary select-none">
